...
$id_spk_encoded = str_replace('/', '_SLASH_', $spk_info->id_spk_penawaran);
?>
...
<div class="box box-primary">
    <div class="box-header with-border">
        <h3 class="box-title"><i class="fa fa-clipboard"></i> Buat Laporan Kunjungan</h3>
    ...
    <div class="box-body">
        <form id="form_visit_report" method="post">
            ...
            <input type="hidden" name="finish_time" id="finish_time" value="">
                ...
                <div class="panel-body">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" value="<?= htmlspecialchars($spk_info->nm_customer) ?>" readonly>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Tanggal</label>
                                <input type="text" class="form-control" value="<?= date('d-m-Y', strtotime($current_date)) ?>" readonly>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Konsultan</label>
                                <input type="text" class="form-control" value="<?= htmlspecialchars($consultant_name) ?>" readonly>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div>
                                    <button type="button" id="btn_start_time" class="btn btn-success btn-sm">
                                        <i class="fa fa-play"></i> Start
                                    </button>
                                    <span id="display_start_time" class="time-display">--:--</span>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div>
                                    <button type="button" id="btn_finish_time" class="btn btn-danger btn-sm">
                                        <i class="fa fa-stop"></i> Finish
                                    </button>
                                    <span id="display_finish_time" class="time-display">--:--</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="panel-body">
                    ...
                    <div id="custom_activities_section">
                        ...
                        <div id="custom_activities_container">
                            <!-- Custom activity rows will be appended here -->
                        </div>
...
$(document).ready(function() {
    ...
    // ========================================
    // On page load: fetch activities and mandays info
    // ========================================
    loadActivities();
    loadMandaysInfo();
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status == 1) {
                    ...
                    btn.prop('disabled', true); // Keep disabled after recording
                } else {
    ...
    function saveReport(saveType) {
        ...
        // Collect action plans (from Task 8.2 section if present)
        if (typeof collectActionPlans === 'function') {
            formData.action_plans = collectActionPlans();
        }

        // Collect improvements (from Task 8.2 section if present)
        ...
        if (formData.action_plans) {
            ...
            // Remove has_error flag from each item
        ...
        $.ajax({
            url: siteurl + 'laporan_kunjungan/save',
            type: 'POST',
            dataType: 'json',
            data: formData,
            success: function(response) {
    ...
    // Load previous action plans on page load
    ...
    $(document).on('click', '.btn-add-action-plan', function() {
        ...
        rowHtml += '<textarea class="form-control ap-description" maxlength="500" rows="2" placeholder="Deskripsi action plan (maks 500 karakter)"></textarea>';
        ...
        rowHtml += '<input type="text" class="form-control ap-pic" maxlength="100" placeholder="Person in charge">';
        ...
        rowHtml += '<input type="date" class="form-control ap-due-date" min="' + todayDate + '">';
        ...
        rowHtml += '<select class="form-control ap-status">';
        rowHtml += '<option value="progress" selected>Progress</option>';
    ...
    $('#btn_add_improvement').on('click', function() {
        ...
        rowHtml += '<textarea class="form-control imp-potensi" rows="2" placeholder="Potensi improvement..."></textarea>';
        ...
        rowHtml += '<textarea class="form-control imp-hasil" rows="2" placeholder="Hasil improvement..."></textarea>';
        ...
        rowHtml += '<div class="form-group">';
        rowHtml += '<label>Status</label>';
        rowHtml += '<select class="form-control imp-status">';
        rowHtml += '<option value="progress" selected>Progress</option>';
        rowHtml += '<option value="done">Done</option>';
        rowHtml += '</select>';
        rowHtml += '</div>';
        ...
        $('.action-plan-activity-group').each(function() {
            ...
            $(this).find('.action-plan-row').each(function() {
                ...
                if (!dueDate) {
                    ...
                    if (dueDate < today) {
                        $row.find('.ap-due-date').css('border-color', '#dd4b39');
                        $row.find('.ap-due-date-error').text('Due date harus hari ini atau setelahnya.').show();
                        rowHasError = true;
                    }
                }
