...
<style>
  ...
  * {
  ...
  .login-alert.info {
    background: linear-gradient(135deg, #3498db22, #2980b911);
    color: #1a5276;
...
<div class="wrapper">
  ...
  if ($flash) {
    ...
    $msg_text = isset($parts[1]) ? $parts[1] : $flash;
    $icon     = ($msg_type == 'danger')  ? '&#9888;' : (($msg_type == 'success') ? '&#10003;' : '&#8505;');
    echo "<div class='login-alert {$msg_type}'><span class='alert-icon'>{$icon}</span><span>{$msg_text}</span></div>";
  }
  ?>
