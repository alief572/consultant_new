...
<script>
    ...
    function hitung_all() {
        ...
        for (i = 1; i <= no; i++) {
            ...
            if(!isNaN(pengajuan_budget_new)) {
                ...
                $('input[name="req_lab[' + i + '][total_budget_tambahan]"]').val(number_format(total_budgett, 2));
    ...
    function add_custom_item() {
        no++;
        var html = '<tr class="custom_lab_' + no + '">';

        html += '<td class="text-center">' + no + '</td>';

        html += '<td>';
        html += '<input type="hidden" name="req_lab[' + no + '][id_detail]" value="">';
        html += '<input type="hidden" name="req_lab[' + no + '][id_item]" value="">';
        html += '<textarea class="form-control form-control-sm" name="req_lab[' + no + '][nm_item]"></textarea>';
        html += '</td>';

        html += '<input type="hidden" name="req_lab[' + no + '][qty_estimasi]" value="0">';
        html += '<input type="hidden" name="req_lab[' + no + '][price_unit_estimasi]" value="0">';
        html += '<input type="hidden" name="req_lab[' + no + '][total_budget]" value="0">';

        html += '<td>';
        html += '<input type="number" class="form-control form-control-sm text-right" name="req_lab[' + no + '][qty_budget_tambahan]" min="0" value="0" onchange="hitung_all()">';
        html += '</td>';

        html += '<td>';
        html += '<input type="text" class="form-control form-control-sm text-right auto_num" name="req_lab[' + no + '][budget_tambahan]" onchange="hitung_all()">';
        html += '</td>';

        html += '<td>';
        html += '<input type="text" class="form-control form-control-sm text-right" name="req_lab[' + no + '][total_budget_tambahan]" readonly>';
        html += '</td>';

        html += '<td>';
        html += '<input type="text" class="form-control form-control-sm text-right" name="req_lab[' + no + '][pengajuan_new_budget]" readonly>';
        html += '</td>';

        html += '<td>';
        html += '<textarea class="form-control form-control-sm" name="req_lab[' + no + '][reason]"></textarea>';
        html += '</td>';
