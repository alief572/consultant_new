            ...
            <table class="table custom-table">
                ...
                <tbody>
                    ...
                    foreach ($list_data_lab as $item) {
                        ...
                        echo '<input type="hidden" name="detail_lab[' . $no . '][nm_item]" value="' . (!empty($item->nm_biaya) ? $item->nm_biaya : $item->nm_item) . '">';
                        echo '</td>';
                    ...
                    foreach ($list_data_lab_custom as $item) {
                        $no++;
                        ...
                        echo '<input type="hidden" name="detail_lab[' . $no . '][total_budget_estimasi]" value="' . $item->estimasi_total . '">';
                        echo '</td>';
                ...
                <tbody>
                    ...
                    foreach ($list_data_lab as $item) {
                        if (isset($list_arr_kasbon[$item->id_lab])) {
                            ...
                            echo '<td>';
                            echo '<input type="text" name="detail_lab[' . $no . '][nominal_pengajuan]" class="form-control form-control-sm text-right auto_num" onchange="hitung_all_pengajuan()" data-no="' . $no . '" data-budget="' . $item->price_unit_final . '" value="' . $nominal_pengajuan . '" ' . $readonly . '>';
                            echo '</td>';
                    ...
                    foreach ($list_data_lab_custom as $item) {
                        if (isset($list_arr_kasbon[$item->id])) {
                            ...
                            echo '<td>';
                            echo '<input type="text" name="detail_lab[' . $no . '][nominal_pengajuan]" class="form-control form-control-sm text-right auto_num" onchange="hitung_all_pengajuan()" data-no="' . $no . '" data-budget="' . $item->estimasi_harga . '" value="' . $nominal_pengajuan . '" ' . $readonly . '>';
                            echo '</td>';
...
<script>
    ...
    $(document).on('submit', '#frm-data', function(e) {
        ...
        if (ttl_propose <= 0) {
            errors.push({ name: 'Item Pengajuan (minimal 1 item harus diajukan dengan nominal > 0)', el: $('input[name="detail_lab[1][qty_pengajuan]"]') });
            $('input[name="detail_lab[1][qty_pengajuan]"]').addClass('input-error');
        }
