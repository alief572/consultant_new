            ...
            <table class="table custom-table">
                ...
                <tbody class="list_informasi_subcont_tenaga_ahli">
                    ...
                    foreach ($list_kasbon_akomodasi as $item) {
                        ...
                        echo $no;
                        echo '<input type="hidden" name="req_subcont_tenaga_ahli[' . $no . '][id_detail]" value="' . $item->id . '">';
                        echo '<input type="hidden" name="req_subcont_tenaga_ahli[' . $no . '][id_item]" value="' . $item->id_item . '">';
                        echo '<input type="hidden" name="req_subcont_tenaga_ahli[' . $no . '][nm_item]" value="' . $item->nm_item . '">';
                        echo '</td>';
                        echo '<td class="text-left">' . $item->nm_item . '</td>';

                        echo '<td class="text-center">';
                        echo number_format($item->qty_final);
                        echo '<input type="hidden" name="req_subcont_tenaga_ahli[' . $no . '][qty_estimasi]" value="' . $item->qty_final . '">';
                        echo '</td>';

                        echo '<td class="text-center">';
                        echo number_format($item->price_unit_final, 2);
                        echo '<input type="hidden" name="req_subcont_tenaga_ahli[' . $no . '][price_unit_estimasi]" value="' . $item->price_unit_final . '">';
                        echo '</td>';

                        echo '<td class="text-center">';
                        echo number_format($item->total_final, 2);
                        echo '<input type="hidden" class="form-control form-control-sm" name="req_subcont_tenaga_ahli[' . $no . '][total_budget]" value="' . $item->total_final . '">';
                        echo '</td>';

                        echo '<td>';
                        echo '<input type="number" class="form-control form-control-sm text-right" name="req_subcont_tenaga_ahli[' . $no . '][qty_budget_tambahan]" min="0" value="0" onchange="hitung_all()">';
                        echo '</td>';

                        echo '<td>';
                        echo '<input type="text" class="form-control form-control-sm text-right auto_num" name="req_subcont_tenaga_ahli[' . $no . '][budget_tambahan]" value="' . $item->price_unit_final . '" readonly>';
                        echo '</td>';

                        echo '<td>';
                        echo '<input type="text" class="form-control form-control-sm text-right auto_num" name="req_subcont_tenaga_ahli[' . $no . '][total_budget_tambahan]" value="0" readonly>';
                        echo '</td>';

                        echo '<td>';
                        echo '<input type="text" class="form-control form-control-sm text-right auto_num" name="req_subcont_tenaga_ahli[' . $no . '][pengajuan_new_budget]" value="' . $item->total_final . '" readonly>';
                        echo '</td>';

                        echo '<td>';
                        echo '<input type="text" class="form-control form-control-sm" name="req_subcont_tenaga_ahli[' . $no . '][reason]">';
                        echo '</td>';
    ...
    $(document).on('submit', '#frm-data', function(e) {
        ...
        swal({
            ...
            if (next) {
                ...
                $.ajax({
                    type: 'post',
                    url: siteurl + active_controller + 'save_request_budget_subcont_tenaga_ahli',
                    data: formData,
                    cache: false,
                    processData: false,
                    contentType: false,
                    ...
                    success: function(result) {
                        if (result.status == '1') {
                            swal({
                                ...
                                window.location.href = siteurl + active_controller + "add_kasbon_subcont_tenaga_ahli/<?= urlencode(str_replace('/', '|', $list_budgeting->id_spk_budgeting)) ?>";
                            });
                        } else {
                            swal({
                                type: 'warning',
