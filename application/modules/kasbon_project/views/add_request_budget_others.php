...
<script>
    ...
    function hitung_all() {
        ...
        for (i = 1; i <= no; i++) {
            ...
            $('input[name="req_others[' + i + '][total_new_budget]"]').val(number_format(total_budgett, 2));

            ttl_pengajuan_new_budget += pengajuan_budget_new;
            ttl_budget += (qty_budget_tambahan * budget_tambahan);

            var price_custom = 0;
            if ($('input[name="custom_others[' + i + '][qty_estimasi_others]"]').length > 0) {
                price_custom = get_num($('input[name="custom_others[' + i + '][harga_estimasi_others]"]').val());
            }

            var qty_custom = 0;
            if ($('input[name="custom_others[' + i + '][qty_estimasi_others]"]').length > 0) {
                qty_custom = $('input[name="custom_others[' + i + '][qty_estimasi_others]"]').val();
            }

            var total_budget_estimasi = get_num($('input[name="req_others[' + i + '][total_budget]"]').val());

            var total_custom = (qty_custom * price_custom);

            if ($('.total_estimasi_others_' + i).length > 0) {
                $('.total_estimasi_others_' + i).autoNumeric('set', total_custom);
            }
    ...
    function add_custom_item() {
        ...
        html += '<td>';
        html += '<textarea class="form-control form-control-sm" name="custom_others[' + no + '][nm_item]"></textarea>';
        html += '</td>';

        html += '<td>';
        html += '<input type="number" class="form-control form-control-sm text-right" min="0" name="custom_others[' + no + '][qty_estimasi_others]" onchange="hitung_all();">';
        html += '</td>';

        html += '<td>';
        html += '<input type="text" class="form-control form-control-sm auto_num text-right" name="custom_others[' + no + '][harga_estimasi_others]" onchange="hitung_all();">';
        html += '</td>';

        html += '<td>';
        html += '<input type="text" class="form-control form-control-sm auto_num text-right total_estimasi_others_' + no + '" name="custom_others[' + no + '][total_estimasi_others]" readonly>';
        html += '</td>';
        ...
        html += '<td>';
        html += '<textarea class="form-control form-control-sm" name="custom_others[' + no + '][reason]"></textarea>';
        html += '</td>';
