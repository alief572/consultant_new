...
function H_activity_record($activity)
{
	$CI = &get_instance();
	...
	$uk	= time() . str_replace(':', '', $CI->input->ip_address());
	$uk	= $uk;
	if ($agent !== 'Googlebot' or $agent !== 'Baiduspider' or $agent !== 'YandexBot') {
		$insert = array(
			'users' 	  => $CI->session->userdata('usr_username'),
			'activity' 	  => $activity,
			'agent' 	  => $agent,
			'platform' 	  => $platform,
			'ip_address'  => $ip_address,
			'time_jkt' 	  => date('Y-m-d H:i:s'),
			'expiry_date' => date('Y-m-d H:i:s', strtotime('+ 30 Minutes')),
			'uk' 		  => $uk
		);
		$masuk = $CI->db->insert('kons_activity_user', $insert);
	}
}

function generateNoPenawaran()
{
	$CI = &get_instance();
	$Ym 			= date('ym');
	$srcMtr			= "SELECT MAX(id_quotation) as maxP FROM kons_tr_penawaran WHERE id_quotation LIKE 'TRS" . $Ym . "%' ";
	$resultMtr		= $CI->db->query($srcMtr)->result_array();
	$angkaUrut2		= $resultMtr[0]['maxP'];
	$urutan2		= (int)substr($angkaUrut2, 7, 4);
	$urutan2++;
